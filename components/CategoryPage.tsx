...
const CategoryPage = ({
  ...
  return (
    <section className="w-full lg:h-[calc(100vh-120px)] lg:overflow-hidden flex flex-col pt-0">
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between shrink-0 px-1">
        <div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-bold uppercase tracking-wider text-gray-700 hover:bg-gray-50 transition-all lg:hidden"
          >
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-bold uppercase tracking-wider text-gray-700 hover:bg-gray-50 transition-all"
            >
            ...
            <div
              className={`absolute right-0 z-50 mt-2 w-56 rounded-xl border border-gray-100 bg-white shadow-2xl shadow-gray-200/50 transition-all duration-300 ${
                sortOpen
            ...
            >
              {[
                ...
                <button
                  ...
                  className={`w-full px-4 py-2.5 text-left text-xs font-bold transition-all first:rounded-t-xl last:rounded-b-xl ${
                    sortBy === item.value
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start relative flex-1 min-h-0">
        {/* Sidebar - Sticky */}
        <aside
          className={`fixed lg:sticky lg:top-0 left-0 top-0 z-[60] h-full lg:h-full w-80 lg:w-[280px] overflow-y-auto no-scrollbar bg-white lg:bg-transparent transition-all duration-300 border-r lg:border-none border-gray-100 p-6 lg:p-0 shrink-0 ${
        ...
        >
          ...
          <div className="space-y-6 lg:bg-white lg:rounded-2xl lg:border lg:border-gray-100 lg:p-5 lg:shadow-sm pb-24 lg:pb-12">
            {/* Categories List */}
            <div>
              <h3 className="text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-3 px-1">Collections</h3>
                ...
                {categories.map((category) => (
                  <div key={category.slug} className="space-y-1">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {category.productCount !== undefined && (
                          <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${categorySlug === category.slug ? "bg-white/20 text-white" : "bg-gray-100 text-gray-400 group-hover:bg-amber-100 group-hover:text-amber-600"}`}>
                    ...
                    {category.children && category.children.length > 0 && (
                      <div className="ml-3 pl-3 border-l border-gray-100 space-y-1 my-1">
                        {category.children.map((child) => (
                          <Link
                            key={child.slug}
                            href={`/category/${child.slug}`}
                            onClick={() => setMobileFilterOpen(false)}
                            className={`flex items-center justify-between rounded-lg px-3 py-1.5 text-[11px] font-bold transition-all duration-300 group ${
            ...
            <div className="pt-6 border-t border-gray-50">
              <h3 className="text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-4 px-1">Budget</h3>
              <div className="grid grid-cols-1 gap-2">
                {[500, 1000, 1500, 10000].map((price) => (
                  <button
                    key={price}
                    ...
                    className={`flex items-center justify-between px-4 py-2.5 rounded-xl text-xs font-bold transition-all border ${
                      priceRange === price
                        ...
                        : "bg-white text-gray-600 border-gray-100 hover:border-amber-500 hover:text-amber-500"
                    }`}
                  >
                    <span>{price === 10000 ? "All Collection" : `Under ₹${price}`}</span>
                    {priceRange === price && <div className="w-1.5 h-1.5 rounded-full bg-white"></div>}
                  </button>
                ))}
              </div>
            </div>
